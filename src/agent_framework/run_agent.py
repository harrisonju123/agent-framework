...
def _build_agent(agent_id: str, workspace: Path) -> Agent:
    """Build a fully configured Agent instance.

    Extracted so both the polling loop (main) and single-task execution
    (run_single_task / CLI run-task) can share the same setup logic.
    """
    ...
    if not agent_def:
        raise ValueError(f"Agent {base_agent_id} not found in config (requested: {agent_id})")
    ...
    if framework_config.llm.mode == "litellm":
        ...
        _logger.info("Using LiteLLM direct backend")
    else:
        ...
        if framework_config.llm.use_mcp and framework_config.llm.mcp_config_path:
            ...
            _logger.info(f"MCP enabled, config path: {mcp_config_path}")

        proxy_env = framework_config.llm.get_proxy_env()
        if proxy_env:
            _logger.info(f"LLM proxy enabled: {framework_config.llm.proxy_url}")
    ...
    if github_token:
        ...
        _logger.info("MultiRepoManager initialized")
        ...
        if worktree_config.enabled:
            ...
            _logger.info(f"WorktreeManager initialized (root: {wt_config.root})")
        ...
        _logger.warning("No GITHUB_TOKEN, multi-repo features disabled")
...
def main():
    ...
    # Support --task flag for single-task execution (used by parallel workers)
    task_id = None
    if "--task" in sys.argv:
    ...
    try:
        agent = _build_agent(agent_id, workspace)
        ...
        if task_id:
            logger.info(f"Single-task mode: executing {task_id}")
            success = asyncio.run(agent.run_single_task(task_id))
