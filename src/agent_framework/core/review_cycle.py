...
Extracted from agent.py to focus on QA review/fix loop operations.
This module manages the automated review cycle between QA and Engineer agents.
"""
...
class ReviewCycleManager:
    """Manages QA review and fix cycles between QA and Engineer agents.

    Handles:
    - Queueing code reviews when PRs are created
    - Parsing QA review outcomes (structured findings, severity, approval)
    - Queueing fix tasks back to Engineer when issues found
    - Escalating to Architect after MAX_REVIEW_CYCLES
    - Purging orphaned review tasks
    """
...
""",
    ...
    def queue_code_review_if_needed(self, task: Task, response, sync_jira_fn=None) -> None:
        """
    ...
    def queue_review_fix_if_needed(self, task: Task, response, sync_jira_fn=None) -> None:
        ...
        if outcome.approved and not outcome.needs_fix:
            if sync_jira_fn:
                sync_jira_fn(task, "Approved", comment=f"QA approved by {self.config.id}")
            return
